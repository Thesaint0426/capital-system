                    ...
                    <td style={{ padding: '13px 14px' }}>
                      {m.current_balance === null ? (
                        <button onClick={() => { setSelectedUser(m); setDeposit('10000'); setShowModal(true); }}
                          style={{ padding: '6px 12px', background: 'transparent', border: '1px solid #252525', borderRadius: 5, color: '#f5f3ef', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans,sans-serif', transition: 'all 0.15s' }}
                          onMouseOver={e => { e.target.style.borderColor = '#00e87a'; e.target.style.color = '#00e87a'; }}
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(6px)', padding: 24 }}>
          <div style={{ background: '#0c0c0c', border: '1px solid #252525', borderRadius: 16, padding: 32, width: '100%', maxWidth: 440 }}>
            <div style={{ fontFamily: 'DM Sans,sans-serif', fontSize: 18, fontWeight: 700, marginBottom: 6, letterSpacing: '-0.01em' }}>Activate Account</div>
            ...
            <form onSubmit={handleCreateAccount} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 8 }}>
                ...
                <button type="submit" disabled={submitting}
                  style={{ padding: '10px 20px', background: submitting ? '#3a3734' : '#00e87a', border: 'none', borderRadius: 7, color: '#000', fontFamily: 'DM Sans,sans-serif', fontSize: 13, fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer' }}>
